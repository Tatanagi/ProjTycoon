...
public class LuckyLoanLender : MonoBehaviour
{
    public void OfferLoan(PlayerController player, int loanAmount)
    {
        if (!player.hasLoan && loanAmount >= 1 && loanAmount <= 5)
        {
            ...
            player.isInDebt = false; // Set isInDebt to false when taking a loan
            Debug.Log($"{player.name} took a loan of {loanAmount} shiny pennies. Must repay {loanAmount * 2} next round.");
        }
        else
        {
